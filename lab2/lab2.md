...
2. ### Объединение одинаковых команд, идущих подряд
   Если одинаковые команды, например, RUN, идут подряд, то их аргументы необходимо объединить с помощью &&. Это позволит создать меньше слоев, улучшив произоводительность и занимаемое место.
   ```
   # плохо
   RUN apt-get update
   RUN apt-get install -y curl
